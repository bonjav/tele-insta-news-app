...
async function updateNotificationStatus(
  ...
  const notifiedTranslationIds = results
    .filter(r => r.translationId)
    .map(r => r.translationId);
  
  const successTickets = successfulResults
    .map(r => r.ticket)
...
async function fetchTranslationWithRetry(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    if (translations && translations.length > 0) {
      return { data: translations[0], error: null };
    }
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: userPrefs, error: userError } = await supabaseClient
      ...
      try {
        if (!pref.push_token || !pref.language_code) {
          ...
          notificationResults.push({
            status: 'error',
            error: { code: 'INVALID_PREFERENCES', message: 'Missing push token or language code' },
        ...
        const { data: translation, error: translationError } = await fetchTranslationWithRetry(
          supabaseClient,
          articleData.id,
          pref.language_code
        );
